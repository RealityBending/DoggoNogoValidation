---
title: "Doggo Nogo Validation - Data Cleaning"
editor: source
# editor_options: 
...
df <- df |> 
  ...
  dplyr::mutate(DoggoNogo_Duration = as.numeric(DoggoNogo_L1_End-DoggoNogo_L1_Start),
                SimpleRT_Duration = ((SimpleRT_end-SimpleRT_start)/1000)/60,
                Gender = stringr::str_to_title(Gender), #ifelse(Gender == "other", "Other", Gender),
                Education = stringr::str_to_title(Education)) |> #ifelse(Education == "other", "Other", Education)) |>
  dplyr::mutate(Assessment_DurationDifference_DoggoNogo = as.numeric(Assessment_Duration_DoggoNogo)-DoggoNogo_Duration,
                Assessment_DurationDifference_Simple = as.numeric(Assessment_Duration_Simple)-SimpleRT_Duration,
...
df_simpleRT <- read.csv("../data/rawdata_simpleRT.csv") |> 
  ...
  dplyr::mutate(RT = ifelse(RT == "null", NA_integer_, as.numeric(RT))) # NA or ISI value?
...
# Incomplete data
dnf_df <- df |> dplyr::filter(is.na(DoggoNogo_ID))
dnf_ppts <- dnf_df$Participant
```
The initial sample of complete data consisted of `r report::report_participants(df |> dplyr::select(Age, Gender, Education), age="Age", gender="Gender", education="Education")`, for a total trial number of `r nrow(df)`. <!-- Only works with dplyr::select due to POSIXct-->

Incomplete data was collected for `r paste(nrow(dnf_df), ifelse(nrow(dnf_df) < 2, "participant", "participants"))` - `r ifelse(nrow(dnf_df) < 2, "this participant", "these participants")` quit the gamified task prematurely and returned to the main experiment screen to complete the procedure. Details of `r ifelse(nrow(dnf_df) < 2, "this participants'", "these participants average")` enjoyment of the experiment and of each task, task duration estimates, and willingness to repeat each task are shown below. Incomplete data is subsequently excluded.
...
dnf_df |> 
  dplyr::summarise(N = n(),
                   Experiment_Enjoyment = mean(Experiment_Enjoyment, na.rm = T),
                   Doggo_Enjoyment = mean(Assessment_Enjoyment_DoggoNogo, na.rm = T),
                   SRT_Enjoyment = mean(Assessment_Enjoyment_Simple, na.rm =T),
                   Dog_Duration = mean(Assessment_Duration_DoggoNogo, na.rm = T), 
                   SRT_Duration = mean(Assessment_Duration_Simple),
                   Dog_Repeat = mean(Assessment_Repeat_DoggoNogo),
                   SRT_Repeat = mean(Assessment_Repeat_Simple)
                   ) |> 
  kableExtra::kable() |> 
  kableExtra::kable_classic()

df <- df |> dplyr::filter(!Participant %in% dnf_ppts)
df_simpleRT <- df_simpleRT |> dplyr::filter(!Participant %in% dnf_ppts)
df_dog <- df_dog |> dplyr::filter(!Participant %in% dnf_ppts)
```
...
df |> 
  ...
  ggplot2::geom_bar(aes(fill = Gender)) +
...
df |>
  ...
  labs(title = "Handedness", y = "Number of Participants", subtitle = "Participants per handedness") +
...
df |>
  dplyr::mutate(Student = ifelse(is.na(Student), "Non-Student", "Student")) |>
  ggplot(aes(x = Student)) +
  geom_bar(aes(fill = Student)) +
  scale_y_continuous(expand = c(0, 0), breaks= scales::pretty_breaks()) +
  scale_fill_viridis_d(guide = "none") +
  labs(title = "Student", y = "Number of Participants", subtitle = "Participants student status") +
  theme_modern(axis.title.space = 15) +
  theme(
    plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0),
    plot.subtitle = element_text(size = rel(1.2), vjust = 7),
...
ggwaffle::waffle_iron(df, ggwaffle::aes_d(group = Discipline), rows=10) |> 
  ggplot(aes(x, y, fill = group)) + 
  ggwaffle::geom_waffle() + 
  coord_equal() + 
  scale_fill_flat_d() + 
  ggwaffle::theme_waffle() +
  labs(title = "Discipline", subtitle = "Each square represents a participant", fill="")  +
  theme(
    plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0),
...
```{r}
ggwaffle::waffle_iron(df, ggwaffle::aes_d(group = Country), rows=10) |> 
  ggplot(aes(x, y, fill = group)) + 
  ggwaffle::geom_waffle() + 
  coord_equal() + 
  scale_fill_flat_d() + 
  ggwaffle::theme_waffle() +
  labs(title = "Country of residence", subtitle = "Each square represents a participant", fill="")  +
  theme(
    plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0),
    plot.subtitle = element_text(size = rel(1.2)),
    axis.title.x = element_blank(),
...
write.csv(df, "../data/data_dog.csv", row.names = FALSE)
write.csv(df, "../data/data_simple.csv", row.names = FALSE)
```
