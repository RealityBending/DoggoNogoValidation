...
We removed `r nrow(rt_exclusions$doggo)` (`r insight::format_percent(nrow(rt_exclusions$doggo) / nrow(df_doggo))`) doggo/nogo trials and `r nrow(rt_exclusions$simple)` (`r insight::format_percent(nrow(rt_exclusions$simple) / nrow(df_simpleRT))`) simple RT trials in which participants responded faster than 150ms or slower than 1000ms.
